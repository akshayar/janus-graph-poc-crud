...
import com.aksh.janusgraph.dto.EdgeIndexVO;
import com.aksh.janusgraph.dto.EdgeVO;
import com.aksh.janusgraph.dto.PropertyIndexVO;
import com.aksh.janusgraph.dto.PropertyVO;
import com.aksh.janusgraph.dto.Schema;

@Component
public class GraphSchemaCreator {
	public static final String INDEX_BACK_END_NAME = "search";
	@Autowired
	private JanusGraphDBInitializer titanDBConfigurator;
	JanusGraph graphDb;
	...
	public void init() {
		logger.info("Init");
		graphDb = (JanusGraph) titanDBConfigurator.getGraph();
//		graphDb.tx().onReadWrite(Transaction.READ_WRITE_BEHAVIOR.MANUAL);
	...
	private Object createProperty(PropertyVO property) {
		...
		if (propertyKey == null) {
			PropertyKeyMaker propertyKeyMaker = mgmt.makePropertyKey(property.getName())
					.dataType(com.aksh.janusgraph.util.TitanDataType.getDataTypeClass(property.getDataType()));
	 ...
	 * titanSchemaType.name() + "_" + propertyKey; JanusGraphIndex graphIndex =
	 ...
	 * createPropertyKey(propertyKeyName, propertyType); String indexName =
	 * schemaTypeName + "_" + propertyKey; JanusGraphIndex graphIndex =
	 * mgmt.getGraphIndex(indexName); if (graphIndex == null) { graphIndex =
